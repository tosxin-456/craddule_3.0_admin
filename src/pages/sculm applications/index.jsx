...
export default function AdminScumlApplications() {
    ...
    const fetchApplications = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/admin/scuml-applications?page=${page}`,
        ...
        if (data.success) {
          ...
          toast.error(data.message || "Failed to fetch scuml applications");
        }
      } catch (err) {
        console.error(err);
        toast.error("Error fetching scuml applications");
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateApplication = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/admin/scuml-applications/${selectedApp.id}/update`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
